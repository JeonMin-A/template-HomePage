    ...
    <div class="basis-full">
        <div class="max-w-7xl mx-auto my-10">
            ...
            <ul class="flex justify-between border-[#20293C] border-t-2 p-4 py-2 bg-[#a6acb9] text-xs sm:text-sm text-center">
                ...
                <li class="basis-[11%] md:basis-[6%]">조회수</li>
            ...
            <template v-for="(e, index) in dataList.slice()" :key="index">
                ...
                <ul v-if="calculatorNumber(totalLength, perPage, page, index) > 0" class="flex justify-between border-b text-center px-4 py-2 even:bg-gray-50 text-xs sm:text-sm">
                    <!-- 게시글 번호 / 가운데 지우면 .slice().reverse()  -->
                    <li class="basis-1/12">{{calculatorNumber(totalLength, perPage, page, index)}}</li> 
                    <!-- 제목 -->
                    <li class="basis-[15%] md:basis-[20%]">
                        <router-link :to="{name: 'NoticeRead', query:{docId: dataId[index]}}" @click="$store.commit('NoticeRead', dataId[index])" class="truncate block w-[60px] mx-auto">{{e.title}}</router-link>
                    </li>
                    <!-- 글쓴이 -->
                    <li class="flex basis-[14%] md:basis-[6%]">
                        <span class="truncate block w-[35px] md:w-[50px]">{{e.name}}</span>
                    </li>
                    <!-- 날짜 -->
                    <li class="basis-[15%] md:basis-[10%] truncate block ">{{BoardDate(index)}}</li>
                    <!-- 조회수 -->
                    <li class="basis-[11%] md:basis-[9%]"><span class="truncate md:block w-[22px] mx-auto">{{e.hit}}</span></li>
            ...
            <div v-if="$store.state.loginToken != null" class="flex justify-end pb-24 mt-5">
                <router-link to="/cs/notice/write" v-if="this.$store.state.uid == this.$store.state.adminUid" class="bg-[#a6acb9] text-black hover:bg-[#20293C] duration-200 py-2 px-4 hover:text-white font-semibold rounded shadow-md focus:outline-none focus:ring-2 focus:ring-opacity-75 text-xs sm:text-sm ">관리자 글쓰기</router-link>
            </div>
        </div>
    </div>
    <!-- 페이지 네이션 -->
