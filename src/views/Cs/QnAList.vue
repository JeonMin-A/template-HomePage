    ...
    <div class="basis-full">
        <div class="max-w-7xl mx-auto my-10">
            ...
            <ul class="flex justify-between border-[#20293C] border-t-2 p-4 py-2 bg-[#a6acb9] text-xs sm:text-sm text-center">
                ...
                <li class="basis-[15%] md:basis-[16%]">제목</li>
                <li class="basis-[14%] md:basis-[8%]">글쓴이</li>
                <li class="basis-[15%] md:basis-[5%]">날짜</li>
                <li class="hidden md:block md:basis-[5.555%]">조회수</li>
                <li class="hidden md:block md:basis-[4.333%]">추천</li>
                <li class="hidden md:block md:basis-[5.333%]">비추천</li>
            </ul>
            <!-- 게시판 목록 -->
            <template v-for="(e, index) in dataList.slice()" :key="index">
                ...
                <ul v-if="calculatorNumber(totalLength, perPage, page, index) > 0" class="flex justify-between border-b text-center px-4 py-2 even:bg-gray-50 text-xs sm:text-sm">
                    <!-- 게시글 번호 -->
                    <li class="basis-[4.444%]">{{calculatorNumber(totalLength, perPage, page, index)}}</li> 
                    <!-- 제목 -->
                    <li class="basis-[15%] md:basis-[16%]">
                        <router-link :to="{name: 'QnARead', query:{docId: dataId[index]}}" @click="$store.commit('QnARead', dataId[index])" class="truncate block w-[70px] md:w-[150px] mx-auto">{{e.title}}</router-link>
                    </li>
                    <!-- 글쓴이 -->
                    <li class="basis-[11%] md:basis-[8%] truncate block w-[40px] md:w-[50px]">{{e.name}}</li>
                    <!-- 날짜 -->
                    <li class="basis-[16%] md:basis-[9%] truncate block md:w-[50px]">{{BoardDate(index)}}</li>
                    <!-- 조회수 -->
                    <li class="hidden md:block md:basis-[5.555%]">{{e.hit}}</li>
                    <!-- 추천 -->
                    <li class="hidden md:block md:basis-[4.333%]">{{e.good}}</li>
                    <!-- 비추천 -->
                    <li class="hidden md:block md:basis-[5.333%]">{{e.bad}}</li>
                </ul>
            </template>
            <div v-if="$store.state.loginToken != null" class="flex justify-end pb-24 mt-5">
                <router-link to="/cs/qna/write" class="bg-[#a6acb9] text-black hover:bg-[#20293C] duration-200 py-2 px-4 hover:text-white font-semibold rounded shadow-md focus:outline-none focus:ring-2 focus:ring-opacity-75 text-xs sm:text-sm ">글쓰기</router-link>
            </div>
        </div>
    </div>
    <!-- 페이지 네이션 -->
