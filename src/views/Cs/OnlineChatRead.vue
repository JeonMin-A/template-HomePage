        ...
        <div class="w-[1000px]">
            <div class="flex justify-between mb-10">
                ...
                <div class="flex gap-x-5">
                    <router-link to="/cs/onlinechat/list" class="bg-[#a6acb9] text-black hover:bg-[#20293C] duration-200 py-2 px-4 hover:text-white font-semibold rounded shadow-md text-xs sm:text-sm mt-5">목록</router-link>
                </div>
                <!-- 상단 - 오른쪽 영역 -->
                <div v-if="$store.state.loginToken != null" class="flex gap-x-5">
                    <router-link to="/cs/onlinechat/modify" class="bg-[#a6acb9] text-black hover:bg-[#20293C] duration-200 py-2 px-4 hover:text-white font-semibold rounded shadow-md text-xs sm:text-sm mt-5">수정</router-link>
            ...
            <div class="flex justify-end mt-7 mb-24">
                <div class="flex gap-x-5">
                    <button @click="GoodChk" class="bg-[#a6acb9] text-black hover:bg-[#20293C] duration-200 py-2 px-4 hover:text-white font-semibold rounded shadow-md text-xs sm:text-sm mt-5 flex items-center gap-2 ">
                        <font-awesome-icon icon="fa-thumbs-up" class="text-xl mt-[-5px]"></font-awesome-icon> : {{BoardContent.good}}
                    </button>
                    <button @click="noGoodChk" class="bg-[#a6acb9] text-black hover:bg-[#20293C] duration-200 py-2 px-4 hover:text-white font-semibold rounded shadow-md text-xs sm:text-sm mt-5 flex items-center gap-2">
                        <font-awesome-icon icon="fa-thumbs-down" class="text-xl mb-[-5px] "></font-awesome-icon> : {{BoardContent.bad}}
                    </button>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        Delete(){
            ...
            if(msg){
                console.log("!!")
                db.collection("onlinechat").doc(this.$route.query.docId).delete().then(()=>{
                    alert("삭제가 완료 되었습니다.");
                    this.$router.replace("/cs/onlinechat")
                })
            }
        },
        BoardDate(index){
        ...
        GoodChk(){
            ...
            db.collection("onlinechat").doc(this.$route.query.docId).update({
                good: this.BoardContent.good + 1,
                goodChk : true
            }).then(()=>{
                db.collection("onlinechat").doc(this.$route.query.docId).get().then((data=>{
    ...
    mounted() {
        ...
        db.collection("onlinechat").doc(this.$route.query.docId).get().then((data)=>{
            ...
            db.collection("onlinechat").doc(this.$route.query.docId).get().then((update)=>{
