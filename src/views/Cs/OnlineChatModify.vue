    ...
    <div class="mx-auto my-10 justify-center">
        <div class="w-[1000px]">
            ...
            <div class="flex justify-end gap-x-5">
                <router-link to="/cs/onlinechat/list" class="bg-[#a6acb9] text-black hover:bg-[#20293C] duration-200 py-2 px-4 hover:text-white font-semibold rounded shadow-md focus:outline-none focus:ring-2 focus:ring-opacity-75 text-xs sm:text-sm mt-5">취소</router-link>
...
export default {
    ...
    methods: {
        Modify(){
            db.collection("onlinechat").doc(this.$store.state.onlinechatId).update({
                "name": this.name,
                "title": this.title,
    ...
    mounted() {
        if(this.$store.state.onlinechatId == 0){
            this.$router.replace("/cs/onlinechat")
        }
        db.collection("onlinechat").doc(this.$store.state.onlinechatId).get().then((data)=>{
